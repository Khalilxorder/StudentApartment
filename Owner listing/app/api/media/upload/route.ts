...
class MediaService {
  ...
  private async generateBlurhash(buffer: Buffer): Promise<string> {
    // Placeholder - would integrate blurhash library
  ...
  private async saveBuffer(buffer: Buffer, path: string): Promise<string> {
    ...
    return `https://storage.supabase.co/${path}`;
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    switch (type) {
      case 'apartment':
        if (!apartmentId) {
          return NextResponse.json({ error: 'apartmentId required for apartment images' }, { status: 400 });
        }
