...
async function calculateCompetitorAdjustment(apartment: any): Promise<number> {
  try {
    ...
    const similarCompetitors = competitors.filter(comp =>
    ...
    const avgCompetitorPrice = similarCompetitors.reduce((sum, comp) => sum + comp.price_huf, 0) / similarCompetitors.length;
