      ...
      {currentResults.length === 0 ? (
        ...
        <>
          ...
          <div 
            ...
            style={{ 
              maxHeight: chatExpanded ? '550px' : chatHovered ? '300px' : '120px', // Show input by default, expand on hover
            }}
          >
            <div className="max-w-7xl mx-auto px-6 py-3">
              
              {/* Chat History - Collapsible, shows when expanded */}
              {messages.length > 0 && (
                <div className={`transition-all duration-300 overflow-hidden ${chatHovered || chatExpanded ? 'max-h-48 mb-3 opacity-100' : 'max-h-0 mb-0 opacity-0'}`}>
                  ...
                  <div className="space-y-2 overflow-y-auto max-h-40">
                    {(chatExpanded ? messages : messages.slice(-6)).map((msg) => ( // Show last 6 messages when expanded
                      <div key={msg.id} className={`flex ${msg.from === 'user' ? 'justify-end' : 'justify-start'}`}>
                        {msg.from === 'ai' && (
