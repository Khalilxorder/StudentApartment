...
export default async function OwnerAnalytics() {
  ...
  const completedBookings = bookings?.filter((b: any) => b.status === 'approved') || [];
  ...
  const monthlyBookings = completedBookings.filter((b: any) =>
    new Date(b.created_at) >= startOfMonth
  );
  const monthlyRevenue = monthlyBookings.reduce((sum: number, b: any) => sum + (b.total_amount || 0), 0);
  ...
  const apartmentStats = apartments?.map((apartment: any) => {
    const apartmentBookings = bookings?.filter((b: any) => b.apartment_id === apartment.id) || [];
    ...
    return {
      ...
      activeBookings: apartmentBookings.filter((b: any) => b.status === 'approved').length,
      revenue: apartmentRevenue,
      occupancyRate: apartmentBookings.length > 0 ?
        (apartmentBookings.filter((b: any) => b.status === 'approved').length / apartmentBookings.length) * 100 : 0
    };
  }) || [];
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {apartmentStats.map((apartment: any) => (
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-medium text-gray-900">
                  {apartmentStats.reduce((sum: number, apt: any) => sum + apt.revenue, 0).toLocaleString()} HUF
                </span>
              </div>
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-medium text-gray-900">
                  {apartmentStats.length > 0 ?
                    apartmentStats.reduce((prev: any, current: any) =>
