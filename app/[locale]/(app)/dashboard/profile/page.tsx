...
export default function ProfilePage() {
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      setMfaEnabled(authUser.user_metadata?.mfa_enabled === true);
    } catch (error) {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Refresh profile state so UI stays in sync
      await loadProfile();
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <div className="text-center">
                ...
                >
                  <div className="relative w-24 h-24 rounded-full overflow-hidden bg-yellow-100 flex items-center justify-center border-2 border-transparent group-hover:border-yellow-400 transition-colors">
                    {profile.avatarUrl ? (
                      <Image
                  ...
                  {mfaEnabled ? (
                    <div className="flex items-center gap-3 text-green-600 bg-green-50 p-4 rounded-lg border border-green-100">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-green-700">Your account is secured with two-factor authentication.</p>
                      </div>
                    </div>
                  ) : (
                    <div>
                      <MFASetup onComplete={() => setMfaEnabled(true)} />
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div className="space-y-4">
                  <p className="text-sm text-gray-600">
                    Manage your personal data in compliance with GDPR. You can export a copy of your data or permanently delete your account.
                  </p>
                  <div className="flex flex-col sm:flex-row gap-4">
                    <button
                      ...
                      className="px-4 py-2 bg-blue-50 text-blue-700 hover:bg-blue-100 font-medium rounded-lg transition border border-blue-200 text-sm"
                    >
                      Export My Data
                    </button>
                    <button
                      ...
                      onClick={async () => {
                        if (confirm('Are you sure you want to delete your account? This action is irreversible.')) {
                          if (confirm('Please confirm one last time. All your data will be permanently removed.')) {
                            try {
                      ...
                      className="px-4 py-2 bg-red-50 text-red-700 hover:bg-red-100 font-medium rounded-lg transition border border-red-200 text-sm"
                    >
                      Delete Account
                    </button>
                  </div>
